    ...
    for file in list_file:
        ...
        while(buffer):
            ...
            if (new_packet.ip1[0:13] == '192.168.100.1'):
                if(new_packet.ip2[0:6] == '192.168'):
                    label = 1
            elif (new_packet.ip2[0:13] == '192.168.100.1'):
                if(new_packet.ip1[0:6] == '192.168'):
                    label = 1
            else:
                label = 0
        
            features = [new_packet.record_time, new_packet.ip1, new_packet.port1, new_packet.ip2, new_packet.port2,
                        new_packet.packet_length, new_packet.proto, src_2_dest_packets, src_2_dest_bytes,
                        dest_2_src_packets, dest_2_src_bytes, total_packets_tran, total_bytes_tran,
                        src_total_packets, src_total_bytes, dest_total_packets, dest_total_bytes, label]
            write_data.append(features)
            buffer = f.read(16)
        
    write_file = pd.DataFrame(write_data, columns = ['Stime','Source IP', 'Source port', 'Dest IP', 'Dest port', 'Packet length', 'Protocol', 'S2D packets', 'S2D bytes', 'D2S packets', 'D2S bytes', 'Total_packets_tran', 'Total_bytes_tran', 'S_total_pkt', 'S_total_by', 'D_total_pkt', 'D_total_by', 'Label'])
