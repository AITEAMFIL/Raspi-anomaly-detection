...
for i in range(len(mlp.coefs_)):
    np.savetxt('mlpcoefs{0}{1}.csv'.format(i + 1,i + 2),mlp.coefs_[i],fmt='%.64f',delimiter=",")
    np.savetxt('mlpbias{0}{1}.csv'.format(i + 1, i + 2), mlp.intercepts_[i], fmt='%.64f',delimiter=",")