...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep 10 17:02:35 2020
...
def packet_sniffer():
    ...
    global packet_extracted
    global time_captured
    global time_extracted
    global time_data
    global conn
    global check
    ...
    while True:
        ...
        if(check == 1):
            packet_extracted = packet_captured
            time_extracted = time_captured
            check = 0
            packet_captured = []
            time_captured = []
        packet_captured.append(data)
        time_captured.append(recordTime)
...
def feature_extraction():
    ...
    while True:
        while(check):
            time.sleep(0.01)
        n = len(time_extracted)
        for i in range(0,n):
            new_packet = packet()
            new_packet.record_time = time_extracted[0]
            del time_extracted[0]
            ether_proto, data = ethernet_frame(packet_extracted[0])
            del packet_extracted[0]
...
def main():
    t1 = threading.Thread(target = packet_sniffer)
    t2 = threading.Thread(target = feature_extraction)
    t1.start()
    t2.start()
